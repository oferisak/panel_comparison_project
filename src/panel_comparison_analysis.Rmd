...
all_rocs_max_f1<-all_rocs%>%group_by(phenotype_name,group,max_dist)%>%slice_max(n=1,order_by = f1,with_ties = F)
...
for (selected_phenotype_name in selected_phenotypes){
  relatedness_df<-selected_phenotypes_naive_vs_auto[[selected_phenotype_name]]$naive_discrepancy$relatedness
...
discrepancy_text<-paste0(glue('For the selected phenotypes, a naive search identified, on average {num_o_panels_summary$numeric.mean} panels per phenotype (IQR [{num_o_panels_summary$numeric.p25},{num_o_panels_summary$numeric.p75}])'))
discrepancy_text
```
