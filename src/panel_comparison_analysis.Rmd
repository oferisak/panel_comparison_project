...
# TODO
- there is a basic problem with the (at least the panelapp panels) in which the panels have duplicate genes in them.. need to understand why
...
phenotype_queries<-c('hypertrophic.*cardiomyopathy|hcm|cardiomyopathy.*hypertrophic',
                     'cholestasis',
                     ...
                     'coloboma',
                     'skeletal.* dysplasia|dysplasia.*skeletal|osteo.*dysplas|dysplas*.osteo',
                     'pancreatitis',
                     'intellectual.*disability|mental.*retard|cognitive.*impair',
                     'osteopetrosis',
                     'ectodermal.*dysplasia|ectoderm|dysplasia.*ectoderm',
                     #'cystic.*renal|polycys.*kidn|renal.*cyst|renal*.ciliop|ciliop.*renal',
                     'arthrogryposis',
                     'holoprosencephaly',
                     #'melanoma',
                     'dilated.*cardio|dcm|cardiomyo.*dilated',
                     'cornea.*dyst|dystr.*cornea',
                     'ichthyosis',
                     'lipodystrophy',
                     'craniosynostosis',
                     'arrhythmia',
...
diseases_knowledge_db<-readr::read_delim('/media/SSD/Bioinformatics/Databases/diseases/human_disease_textmining_full.tsv',col_names = c('gene_id','gene_symbol','disease_id','disease_name','z_score', 'confidence_score','url'))
all_diseases<-diseases_knowledge_db%>%pull(disease_name)%>%unique()
head(diseases_knowledge_db)

knowledge_vs_phenotype<-list()
for (selected_phenotype in selected_phenotypes){
  message(glue('Retrieving {selected_phenotype} genes from disgenet'))
  phenotype_query<-selected_phenotypes_df%>%filter(phenotype_name==selected_phenotype)%>%pull(query)
  knowledge_diseases<-grep(phenotype_query,diseases_knowledge_db$disease_name,ignore.case = T,value=T)
  # dsi is a score for gene-disease specificity
  knowledge_gene<-diseases_knowledge_db%>%filter(disease_name %in% knowledge_diseases)%>%group_by(gene_symbol)%>%slice_max(n=1,order_by=confidence_score)
  knowledge_vs_phenotype[[selected_phenotype]]<-knowledge_gene
}

z<-knowledge_vs_phenotype$`hypertrophic cardiomyopathy`
```

```{r generate auto panels}
auto_generator_panelapp_only<-list()
auto_generator_panelapp_with_similar<-list()
max_dists=c(0.85,0.9,0.95,0.99)
...
all_aucs<-auto_generator_rocs%>%group_by(phenotype_name,group)%>%distinct(pr_auc)%>%
...
for (selected_phenotype_name in selected_phenotypes_df$phenotype_name){
  ...
  all_genes_vs_phenotype<-data.frame(gene_symbol=all_genes)%>%
    left_join(knowledge_vs_phenotype[[selected_phenotype_name]])%>%
    mutate(has_pub=factor(ifelse(z_score>=z_score_thresh,1,0)))
...
# compare rocs auto generator 
all_rocs%>%filter(group%in%c('ag_0.85'))%>%
  ggplot(aes(x=recall,y=precision,col=factor(group)))+
  geom_line()+facet_wrap(phenotype_name~.)+
  geom_abline(slope = -1,intercept = 1,linetype=2,alpha=0.5)+
  geom_point(data=panelapp_roc,inherit.aes = F,aes(x=recall,y=precision))+
...
better_recall_vs_panelapp <-
  ...
  filter(phenotype_name %in% (precision_comp %>% filter(abs(distance_matched_to_panelapp_percision -
...
better_precision_vs_naive <-
  precision_comp %>% filter(distance_matched_to_naive_recall > naive) %>%
  filter(phenotype_name %in% (recall_comp %>% filter(abs(distance_matched_to_naive_recall -
...
library(report)
precision_distance_vs_naive_recall_matched<-t.test(precision_comp%>%pull(distance_matched_to_naive_recall),precision_comp%>%pull(naive),paired = T)
...
t.test(recall_comp%>%pull(distance_matched_to_naive_recall),recall_comp%>%pull(naive),paired = T)
...
precision_comp%>%select(distance_matched_to_panelapp_percision,panelapp)%>%colMeans()
t.test(precision_comp%>%pull(distance_matched_to_panelapp_percision),precision_comp%>%pull(panelapp),paired = T)
