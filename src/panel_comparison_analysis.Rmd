...
# Summarize the performance of panelapp vs naive vs auto
methods_performance_metrics<-selected_phenotypes_analysis_df%>%
  mutate(group=ifelse(grepl('^panelapp',group),'panelapp',group))%>%
  filter(grepl('naive|panelapp$|max_dist|^panelapp|cluster|auto',group))%>%group_by(group)%>%
...
for (selected_phenotype_name in selected_phenotypes_df$phenotype_name){
    ...
    all_rocs<-all_rocs%>%
      bind_rows(data.frame(phenotype_name=selected_phenotype_name,
                           ...
                           table_for_roc_panelapp_only%>%
                             pr_curve(truth=has_pub,estimate=scaled_gene_score,event_level = 'second')))%>%
      # bind_rows(data.frame(phenotype_name=selected_phenotype_name,
  ...
  naive_genes<-selected_phenotypes_analysis[[selected_phenotype_name]]$naive_discrepancy_genes
  naive_roc<-naive_roc%>%
    bind_rows(data.frame(phenotype_name=selected_phenotype_name,
...
all_rocs<-all_rocs%>%mutate(f1=(2 * precision * recall) / (precision + recall))

all_rocs_max_f1<-all_rocs%>%filter(group=='gene_score')%>%group_by(phenotype_name,group,max_dist)%>%slice_max(n=1,order_by = f1,with_ties = F)

# add the max f1 stats for all the panels to the metrics df
selected_dist<-0.85
all_rocs_to_add<-all_rocs%>%filter(max_dist==selected_dist,group=='gene_score')

# add number of genes
library(listr)
auto_panels_genes<-auto_generator_panelapp_only$`max_dist=0.85` %>%list_name_to_df()%>%list_bind(selected_phenotypes)
...
for (selected_phenotype in selected_phenotypes){
  ...
  threshs <- all_rocs_to_add %>%
    filter(phenotype_name == selected_phenotype) %>% pull(.threshold)
...
all_rocs_to_add<-all_rocs_to_add%>%
  left_join(ngenes_to_add)

# now for each phenotype, get the recall of the panelapp panel
comparison_metrics<-selected_phenotypes_analysis_df%>%filter(grepl('panelapp',group))%>%
  filter(!grepl('no panelapp',group))%>%
  mutate(group=ifelse(grepl('^panelapp',group),'panelapp',group))%>%
  select(-contains('rank'))%>%
  rename(recall=sensitivity,precision=ppv,phenotype_name=phenotype)
comparison_metrics

panelapp_metrics<-comparison_metrics%>%filter(grepl('panelapp',group))
auto_matched_to_panelapp_by_precision<-all_rocs_to_add%>%left_join(panelapp_metrics,by='phenotype_name')%>%
...
auto_matched_to_naive_by_recall<-all_rocs_to_add%>%left_join(naive_metrics,by='phenotype_name')%>%
  group_by(phenotype_name)%>%
...
comparison_metrics<-
  ...
  bind_rows(all_rocs_to_add%>%group_by(phenotype_name)%>%
  ...
  bind_rows(auto_matched_to_panelapp_by_precision%>%
              ...
              mutate(group='distance_panelapp_percision'))%>%
  bind_rows(auto_matched_to_naive_by_recall%>%
              select(phenotype_name,ngenes=ngenes.x,group=group.x,f1=f1.x,precision=precision.x,recall=recall.x)%>%
              mutate(group='distance_naive_recall'))%>%
  mutate(group=ifelse(grepl('^naive',group),'naive',group))

# plot max f1 vs panelapp vs naive
comparison_metrics%>%ggplot(aes(x=group,y=f1,fill=group))+
  geom_col()+coord_flip()+
  facet_wrap(phenotype_name~.,ncol = 1)+
...
all_rocs%>%filter(max_dist==0.85,grepl('gene_score$',group))%>%ggplot(aes(x=recall,y=precision,col=factor(group)))+
  ...
  geom_point(data=all_rocs_max_f1%>%filter(max_dist==0.85,grepl('gene_score$',group)),inherit.aes = F,aes(x=recall,y=precision),col='blue')+
...
comparison_metrics_summary<-comparison_metrics%>%group_by(group)%>%select(precision,recall,f1)%>%skimr::skim_without_charts()
...
comparison_text<-paste0(glue('Compared to a literature search, PanelApp panels demonstrated '),
                        ...
                        paste0(comparison_metrics_text%>%filter(group=='gene_score')%>%pull(metric_text), 
                        collapse=', '),
...
model_t <- map(.x = params_list,
               .f = ~ tidy(wilcox.test(
                 formula = precision ~ group,
                 paired=T,
                 data    = subset(comparison_metrics, group %in% .x),
                 conf.int=T
               )))
