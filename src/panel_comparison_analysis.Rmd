...
gtr_binary_panel_gene_table<-create_binary_gene_table(gtr_with_panelapp_db)
# filter out the hpo data
#binary_panel_gene_table_no_hpo<-binary_panel_gene_table%>%filter(!grepl('hpo',row.names(binary_panel_gene_table)))
# generate distance matrix between all the panels
...
gen_source_db<-gtr_db
gen_dm_df<-gtr_dm_df%>%filter(!grepl('panelapp|clingen|expert',row))%>%filter(!grepl('panelapp|clingen|expert',col))
