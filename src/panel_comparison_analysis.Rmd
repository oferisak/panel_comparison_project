...
naive_vs_auto_test<-function(panelapp_panel,phenotype_name,naive_search,all_panel_names){
  
  # first get the number of publications for all the genes
  all_genes_vs_phenotype<-pubmed_phenotype_vs_all_genes(phenotype_name,gtr_with_expert_db)
  #load('./data/pre_generated_data/all_genes_vs_hcm.RData')
  all_genes_with_pheno_pub<-all_genes_vs_phenotype%>%filter(pub_num>0)%>%pull(gene_symbol)
  ...
  for (pheno_panel in pheno_naive_panels){
    ...
    all_metrics<-all_metrics%>%bind_rows(as.data.frame(metrics_pheno_panel))
  }
  
  # then claculate performance for all panels
  
  naive_discrepancy<-panels_discrepancy_analysis(phenotype_name,
                                               pheno_naive_panels,
                                               gtr_with_expert_db,
                                               panelapp_panel,
  ...
  metrics_naive<-calculate_sens_and_ppv_vs_pubmed(naive_discrepancy$relatedness$gene_list,all_genes_with_pheno_pub)
  metrics_naive$group='naive_search'
  all_metrics<-all_metrics%>%bind_rows(as.data.frame(metrics_naive))
  ...
  #       
  #       cluster_discrepancy<-panels_discrepancy_analysis(phenotype_name,
  #                                                    cluster_panels,
  #                                                    gtr_with_expert_db,
  #                                                    panelapp_panel,
  #                                                    all_genes_vs_pubmed = all_genes_vs_phenotype)
  #       
  #       metrics_cluster<-calculate_sens_and_ppv_vs_pubmed(cluster_discrepancy$relatedness$gene_list,all_genes_with_pheno_pub)
  #     
  #       metrics_cluster$group=glue('method={clust_method},cluster_size={cluster_size}')
  #     
  #       all_metrics<-all_metrics%>%bind_rows(as.data.frame(metrics_cluster))
  ...
  # check a range of distances
  #all_metrics<-NULL
  for (max_dist in c(0.65,0.75,0.85,0.95)){
    ...
    dist_discrepancy<-panels_discrepancy_analysis(phenotype_name,
                                                 dist_panels,
                                                 gtr_with_expert_db,
                                                 panelapp_panel,
                                                 all_genes_vs_pubmed = all_genes_vs_phenotype)
    
    metrics_dist<-calculate_sens_and_ppv_vs_pubmed(dist_discrepancy$relatedness$gene_list,all_genes_with_pheno_pub)
    metrics_dist$group=glue('max_dist={max_dist}')
...
cholestasis_comp<-naive_vs_auto_test(phenotype_name='cholestasis',
                                     panelapp_panel = "panelapp|544|Cholestasis",
                                     naive_search = 'cholesta',
                                     all_panel_names = all_panel_names)

hcm_comp<-naive_vs_auto_test(phenotype_name='hypertrophic cardiomyopathy',
                                     panelapp_panel = "panelapp|49|Hypertrophic cardiomyopathy - teen and adult" ,
                                     naive_search = 'hcm|hypertrophic',
                                     all_panel_names = all_panel_names)

arrhythmia_comp<-naive_vs_auto_test(phenotype_name = 'cardiac arrhythmia',
                                    panelapp_panel = "panelapp|842|Cardiac arrhythmias",
                                    naive_search='arrhythmia',
                                    all_panel_names = all_panel_names)
```
...
rentrez::set_entrez_key('9d71388851dd03b4991b6e25de46c7da7c08')
httr::set_config(config = add_headers(timeout=50000))

selected_phenotypes_naive_vs_auto<-list()
for (selected_phenotype_name in selected_phenotypes[12:15]){
...
for (selected_phenotype in names(selected_phenotypes_naive_vs_auto)){
  selected_phenotypes_naive_vs_auto_df<-selected_phenotypes_naive_vs_auto_df%>%
    bind_rows(data.frame(phenotype=selected_phenotype,
                         selected_phenotypes_naive_vs_auto[[selected_phenotype]]%>%
                           mutate(rank=rank(desc(ppv),ties.method = 'min'))))
}
z<-selected_phenotypes_naive_vs_auto_df%>%group_by(group)%>%
  summarize(skimr::skim_without_charts(rank))
