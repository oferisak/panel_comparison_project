...
autopanel_scores%>%
  ...
  select(phenotype_name,has_pub,num_o_panels,ag_score)%>%
...
for (selected_phenotype in selected_phenotypes){
  ...
  all_genes_has_pub<-all_genes_has_pub%>%
    bind_rows(data.frame(all_genes_vs_phenotype%>%select(-pubmed_ids),
                ...
                left_join(data.frame(ddb_vs_phenotype[[selected_phenotype]]%>%
                                       select(gene_symbol,confidence_score),
                                     .group=selected_phenotype)))
  
  # all_genes_has_pub<-all_genes_has_pub%>%
  #     left_join(data.frame(ddb_vs_phenotype[[selected_phenotype]]%>%select(gene_symbol,confidence_score),
  #                          .group=selected_phenotype))
}

# now join them
genes_with_score_and_pub<-all_genes_has_pub%>%
  left_join(ag_panels%>%select(gene_symbol,.group,in_original_panels,scaled_gene_score,rank_percentile))%>%
  mutate(scaled_gene_score=ifelse(is.na(scaled_gene_score),0,scaled_gene_score),
         confidence_score=ifelse(is.na(confidence_score),0,confidence_score),
         rank_percentile=ifelse(is.na(rank_percentile),0,rank_percentile),
         rank_percentile_cat=as.character(cut(rank_percentile,breaks=c(0,0.5,0.8,0.9,1),labels=c('Not likely','Mild','Medium','High'))),
         rank_percentile_cat=ifelse(is.na(rank_percentile_cat),'Not likely',rank_percentile_cat))

a<-genes_with_score_and_pub%>%
  group_by(.group,rank_percentile_cat)%>%
  summarize(n=n(),
            in_panelapp=sum(as.numeric(as.character(in_original_panels))),
            has_pub=mean(as.numeric(as.character(has_pub))),
            confidence_score=median(confidence_score))
a

genes_with_score_and_pub%>%
  filter(rank_percentile!=0)%>%
  ggplot(aes(x=rank_percentile_cat,y=confidence_score,col=has_pub))+
  geom_boxplot()#+facet_wrap(.group~.,scales='free')

genes_with_score_and_pub%>%
  filter(rank_percentile!=0)%>%
  ggplot(aes(x=rank_percentile_cat,fill=has_pub))+
  geom_bar()+facet_wrap(.group~.,scales='free')
...
for (reference_source in c('pubmed')){# cannot use this part in the ddb comparison, because the results are divided per confidence thresh, need to create another list just for it
  ...
  for (selected_dist in max_dists){
    ...
    auto_panels_genes<-auto_panels_genes[[1]]%>%select(phenotype_name=.group,gene_symbol,ag_score)
