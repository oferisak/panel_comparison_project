...
pubmed_gene_list_vs_phenotype<-function(phenotype_name,gene_list){
  ...
  phenotype_search_res<- entrez_search(db="pubmed", term=phenotype_query,retmax=99999)$ids
  # Get the gene IDs
  ...
  nchunks<-round(length(ids)/300)+1
...
summarize_panel_vs_phenotype_pubmed<-function(selected_panels,
  ...
  if (!is.na(panelapp_panel)){
    panelapp_genes<-gtr_with_expert_db%>%
      filter(panel_joined_name==panelapp_panel,confidence_level>=min_conf_level)%>%pull(gene_symbol)
